...
def _vault_candidates() -> "list[str]":
    ...
    bothered to look at". Measured on the reporting box: one vault sat at
    ~/AdelaidaNotes and a SECOND real Obsidian vault (8,956 notes) sat at
    ~/Documents/Adelaida Notes with an EMPTY meta/scripts and no preflight. The
    narrow scan saw one candidate, called it unambiguous, and returned 'ok' for
    an account that genuinely had a gap. A wider search does not make discovery
